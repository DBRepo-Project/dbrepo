...
class S3Client:
    ...
    def __init__(self):
        ...
        self.client = boto3.client(service_name='s3', endpoint_url=self.endpoint_url, aws_access_key_id=self.access_access_key_id,
    ...
    def download_file(self, filename) -> bool:
        ...
        filepath = os.path.join("/tmp/", filename)
        try:
            self.client.download_file(self.bucket, filename, filepath)
            logging.info(f"Downloaded .csv with key {filename} into {filepath} from bucket {self.bucket}")
            return True
        except ClientError as e:
            logging.error(e)
            return False
