...
public class DataServiceGatewayImpl implements DataServiceGateway {
    ...
    @Override
    public ViewDto createViewRaw(UUID databaseId, String internalName, String query) throws DataServiceConnectionException,
        ...
        try {
            record CreateReplicatedPayload(String internalName, String query) {}
            response = restTemplate.exchange(path, HttpMethod.POST,
                    new HttpEntity<>(new CreateReplicatedPayload(internalName, query)), ViewDto.class);
        } catch (HttpServerErrorException e) {
