...
public class QueryServiceGatewayImpl implements QueryServiceGateway {
    ...
    public QueryDto find(Long containerId, Long databaseId, IdentifierCreateDto identifier, String authorization) throws QueryNotFoundException,
        ...
        final String url =
                "/api/container/" + containerId + "/database/" + databaseId + "/query/" + identifier.getQueryId();
        ...
        if (response.getStatusCode().equals(HttpStatus.NOT_FOUND)) {
            log.error("Query not found with id {}", identifier.getQueryId());
            log.debug("query not found for identifier {}", identifier);
            throw new QueryNotFoundException("Query not found");
        }
        if (response.getStatusCode().equals(HttpStatus.UNAUTHORIZED)) {
            log.error("Query not authorized with id {}", identifier.getQueryId());
