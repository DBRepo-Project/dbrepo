...
public class QueryStoreServiceImpl extends HibernateConnector implements QueryStoreService {
    ...
    public void create(Long containerId, Long databaseId) throws DatabaseNotFoundException, DatabaseConnectionException {
        ...
        execute(connection, "CREATE TABLE `qs_queries` (`id` bigint not null primary key default nextval(`qs_queries_seq`), `cid` bigint not null, `created` datetime not null default now(), `created_by` bigint not null, `dbid` bigint not null, `execution` datetime not null, `last_modified` datetime, `query` text not null,  `query_normalized` text not null, `query_hash` varchar(255) not null, `result_hash` varchar(255), `result_number` bigint)");
