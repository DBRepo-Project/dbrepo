...
public interface TableMapper {
    ...
    @Mappings({
            ...
            @Mapping(target = "concept",  expression="java(conceptToConceptDto(data.getColumnConcept().getConcept()))")
    })
    default ColumnDto tableColumnToColumnDto(TableColumn data) {
        ColumnDto columnDto = ColumnDto.builder()
                .id(data.getId())
                .name(data.getName())
                .internalName(data.getInternalName())
                .autoGenerated(data.getAutoGenerated())
                .isPrimaryKey(data.getIsPrimaryKey())
                .columnType(columnTypeToColumnTypeDto(data.getColumnType()))
                .unique(data.getIsUnique())
                .isNullAllowed(data.getIsNullAllowed())
                .checkExpression(data.getCheckExpression())
                .foreignKey(data.getForeignKey())
                .references(data.getReferences())
                .enumValues(data.getEnumValues().toArray(String[]::new))
                .build();
        if(data.getColumnConcept() != null && data.getColumnConcept().getConcept() != null) {
            columnDto.setConcept(conceptToConceptDto(data.getColumnConcept().getConcept()));
        }
        if(data.getDateFormat() != null) {
            columnDto.setDateFormat(
                    ImageDateDto.builder()
                            .id(data.getDateFormat().getId())
                            .createdAt(data.getCreated())
                            .databaseFormat(data.getDateFormat().getDatabaseFormat())
                            .unixFormat(data.getDateFormat().getUnixFormat())
                            .example(data.getDateFormat().getExample())
                            .build());
        }
        return columnDto;
    }
